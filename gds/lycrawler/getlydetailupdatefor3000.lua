...
-- base_flights_city
local con = assert (env:connect("qiyou", "rhomobi_dev", "b6x7p6b6x7p6", "localhost", 3306))
...
local sqlcmd = "SELECT `sLyId`, `id`, `blon` FROM `scenery`";
local cur = assert (con:execute(sqlcmd))
local row = cur:fetch ({}, "a")
local citycn = {};
local nums = 0
while row do
	if row.blon == nil then
...
for kid,vsLyId in pairs(citycn) do
	...
	local Scenerybody = ([=[<sceneryId>%s</sceneryId><cs>2</cs>]=]):format(vsLyId)
		...
		if idx1 ~= nil and idx2 ~= nil then
			...
			if xscene ~= nil then
				...
				if rspCode == "0000" then
					...
					resly["bookFlag"] = tonumber(sd["bookFlag"])
					if sd["gradeId"] ~= nil then
						resly["grade"] = string.len(sd["gradeId"])
					end
					if sd["address"] ~= nil then
						resly["address"] = string.sub(sd["address"], 10, -4)
					end
					-- print(sd["city"])
					...
					local body, code, headers, status = http.request {
					-- local ok, code, headers, status, body = http.request {
						url = "http://localhost:1337/scenery/" .. kid,
						-- proxy = "http://10.123.74.137:808",
						-- proxy = "http://" .. tostring(arg[2]),
						timeout = 3000,
					...
					if code == 200 then
						...
						-- print(kid,resly["blon"],resly["blat"])
						print(status)
						print(body)
					else
						print(code)
