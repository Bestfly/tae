	...
	if idx1 ~= nil and idx2 ~= nil then
		...
		if rspCode == "0000" then
			...
			for i = 1, table.getn(pl) do
				...
				if code == 200 then
					print(code .. "----effect data from LY---------\n");
					print("+++++++++++++++");
						...
						if idx1 ~= nil and idx2 ~= nil then
							...
							if rc == "0000" then
								...
								for i = 1, table.getn(cl) do
									local tdbi = "http://localhost:1337/city/create?CountryCode=%s&name=%s&ename=%s&prefixLetter=%s";
									tdbi = string.format(tdbi, "CN", urlencode(pl[i]["name"]), urlencode(pl[i]["enName"]), pl[i]["prefixLetter"])
									local body, code, headers = http.request(tdbi)
							else
								print(error003(rc))
							end
						end
					...
					print(code .. "----write data from LY Failure--\n");
					print(dbi)
				end
				sleep(1)
			end
