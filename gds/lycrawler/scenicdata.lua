...
function GetScenery(sceneryId,ServiceName)
	local ts = os.date("%Y-%m-%d %X", os.time()) .. ".000";
	...
	-- print(signstr)
	local signmd5 = md5.sumhexa(signstr)
	local Scenerybody = ([=[<sceneryId>%s</sceneryId>]=]):format(sceneryId)
	...
	if idx1 ~= nil and idx2 ~= nil then
		...
		if rspCode == "0000" then
			...
			for i = 1, table.getn(sl) do
				...
				local resxml, rescode = GetScenery(sl[i]["sceneryId"],GetSceneryDetail)
				if rescode ~= 1 then
					local pr_xml = xml.eval(resxml);
					local xscene = pr_xml:find("response");
					-- xscene maybe nil
