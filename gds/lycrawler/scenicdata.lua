...
-- local sqlcmd = "SELECT `cLyId`, `id` FROM `city` order by cLyId desc";
		...
		if idx1 ~= nil and idx2 ~= nil then
			...
			if rspCode == "0000" then
				...
				for i = 1, table.getn(sl) do
					...
					-- 初始化待处理为1
					local res, err = client:hget('sce:lycom:city:' .. keyid, sl[i]["sceneryId"])
					print("+++++++++++++++++")
					print(res)
					print("+++++++++++++++++")
					if tonumber(res) ~= 0 then
						...
						print("-----Already Writed to Mysql " .. keyid .. ":" .. sl[i]["sceneryId"] .. "-------")
					end
				end
			else
				print(error003(resxml))
