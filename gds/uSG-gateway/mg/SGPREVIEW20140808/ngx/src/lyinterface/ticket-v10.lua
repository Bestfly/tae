...
local verifykey = "hq5P826n55x3LkwK5k88S5b3XS4h30bTRp";
	...
	if harg["Auth-Timestamp"] ~= nil and harg["Auth-Signature"] ~= nil then
		if math.abs(harg["Auth-Timestamp"] - os.time()) >= 3600 then
			...
			if ngx.md5(verifykey .. harg["Auth-Timestamp"]) ~= harg["Auth-Signature"] then
				...
				local sceneid = tonumber(ngx.var.sceneid)
				local startdate = "";
				local req = "";
				if sceneid ~= nil and sceneid > 0 then
					local srvname = string.lower(ngx.var.srvname);
					if srvname == "scenerylist" then
						srvname = GetSceneryList
						req = ([=[<clientIp>127.0.0.1</clientIp>
						<cityId>%s</cityId><pageSize>100</pageSize>]=]):format(sceneid);
					else
						if srvname == "scenerydetail" then
							srvname = GetSceneryDetail
							req = ([=[<sceneryId>%s</sceneryId>]=]):format(sceneid);
						else
							if srvname == "scenerytraffic" then
								srvname = GetSceneryTrafficInfo
								req = ([=[<sceneryId>%s</sceneryId>]=]):format(sceneid);
							else
								if srvname == "nearbyscenery" then
									srvname = GetNearbyScenery
									req = ([=[<sceneryId>%s</sceneryId>]=]):format(sceneid);
								else
									if srvname == "imagelist" then
										srvname = GetSceneryImageList
										req = ([=[<sceneryId>%s</sceneryId>]=]):format(sceneid);
									else
										if srvname == "sceneryprice" then
											srvname = GetSceneryPrice
											req = ([=[<showDetail>2</showDetail>
											<sceneryIds>%s</sceneryIds>
											<useCache>0</useCache>]=]):format(sceneid);
										else
											if srvname == "pricecalendar" then
												if harg["startDate"] ~= nil then
													srvname = GetPriceCalendar
													startdate = harg["startDate"]
													req = ([=[<policyId>%s</policyId>
													<startDate>%s</startDate>
													<showDetail>1</showDetail>]=]):format(sceneid, startdate);
												else
													ngx.print(error004)
													return
												end
					...
					if srvname ~= JSON.null and srvname ~= nil then
						...
						</request>]=]):format(xv, ad, srvname, signmd5, ts, req)
						local hc = http:new()
