						...
						if harg["If-Match"] == 'sort' then
						    ...
						    for key, val in pairs(parg) do
								if red:exists("avh:" .. key) ~= 0 then
									local res, err = red:zrange(key, 0, 0, "WITHSCORES")
							    ...
							    for key, val in pairs(parg) do
									...
									if red:exists("avh:" .. key) ~= 0 then
										...
										if not res then
											...
											ngx.log(ngx.ERR, error003(table.getn(res) .. "|" .. JSON.encode(res)));
											if table.getn(res) ~= 0 then
												local i = 1;
												while i < table.getn(res) do
													local t = {}
					...
					if pcontent.dt ~= nil and pcontent.uk ~= nil and pcontent.sn ~= nil then
						if pcontent.dt == 10 then
							...
							if idx2 == 45 and string.len(idx7) == 8 and string.len(idx8) == 8 and string.len(idx9) == 6 then
								...
								if not res then
									ngx.print(error003("failed to save vb->>" .. tkey .. '|' .. hid))
									...
									ngx.print(error000("Sucess to save vb->>" .. tkey .. '|' .. hid))
										...
										if not res then
											...
											if pcontent.vb ~= nil then
												if pcontent.vb ~= "" then
													...
													if not ok then
														ngx.print(error003("failed to save vb->>" .. idx3 .. tkey .. '|' .. pcontent.uk))
														ngx.log(ngx.ERR, error003("failed to save vb->>" .. idx3 .. tkey .. '|' .. pcontent.uk .. '|' .. pcontent.vb))
														return
													else
														ngx.print(error000("Sucess to save vb->>" .. idx3 .. tkey .. '|' .. pcontent.uk))
