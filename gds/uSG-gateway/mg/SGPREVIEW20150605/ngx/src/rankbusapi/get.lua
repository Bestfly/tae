...
-- nosql connect
local ok, err = red:connect("10.169.210.134", 61398)
if not ok then
...
local ok, err = memc:connect("10.251.227.237", 51978)
if not ok then
		...
		if math.abs(harg["Auth-Timestamp"] - os.time()) >= 3600 and (harg["Auth-Appid"] ~= appidc01 or harg["Auth-Appid"] ~= appidc02) then
			...
			if ngx.md5(verifykey .. harg["Auth-Timestamp"] .. harg["Auth-Appid"]) ~= harg["Auth-Signature"] then
				...
				if harg["If-Match"] == 'sort' then
					-- local res, err = red:zexists(ngx.var.s1 .. ":" .. ngx.var.s2, ngx.var.uk)
					-- ngx.log(ngx.ERR, res, err, ngx.var.s1 .. ":" .. ngx.var.s2, ngx.var.uk)
					-- if res ~= 1 then
						-- ngx.print(error003("failed to get: " .. ngx.var.s1 .. ":" .. ngx.var.s2 .. "/" .. ngx.var.uk, err))
						-- return;
					local restt = memc:get(ngx.var.s1 .. ngx.md5(ngx.var.s1 .. ":" .. ngx.var.s2 .. ":" .. ngx.var.uk))
					local t = {};
